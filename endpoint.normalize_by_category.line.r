...
for (category in categories){
  categoryPositions<-subset(allGroupPosition, Category==category)
  categoryCount<-unlist(countTable[category,])
  categoryPositions$PositionCountFraction<-as.vector(1000000 * categoryPositions$PositionCount / categoryCount[categoryPositions$File])
  allCategoryPosition<-rbind(allCategoryPosition, categoryPositions)
}
...
m<-ggplot() +
  ...
  theme_bw()+
  ylab("average read coverage (per category reads)")+
  xlim(-10, 10) +
